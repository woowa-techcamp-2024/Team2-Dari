...
// 페스티벌 상세 조회
function getFestivalDetail(festivalId) {
...
// 대기열 참여
function joinQueue(festivalId, ticketId, sessionCookie) {
    const response = http.post(`${WAIT_URL}/festivals/${festivalId}/tickets/${ticketId}/purchase/wait`, JSON.stringify({}), {
        headers: {
    ...
    const checkRes = check(response, {
        'status is 200': (r) => r.status === 200,
        'response has waitOrderBlock': (r) => {
            try {
                const body = JSON.parse(r.body);
                return body && body.data && body.data.waitOrderBlock;
            } catch (e) {
                console.error('Failed to parse joinQueue response:', e);
                return false;
            }
        },
    });
    ...
    if (!checkRes) {
        console.error('joinQueue failed:', response.status, response.body);
    }
...
function getQueuePosition(festivalId, ticketId, sessionCookie, waitOrder) {
    ...
    const checkRes = check(response, {
        'status is 200': (r) => r.status === 200,
        'response has waitOrder': (r) => {
            try {
                const body = JSON.parse(r.body);
                return body && body.data && body.data.purchasable !== undefined;
            } catch (e) {
                console.error('Failed to parse getQueuePosition response:', e);
                return false;
            }
        },
    });
    ...
    if (!checkRes) {
        console.error('getQueuePosition failed:', response.status, body);

    }

    return checkRes ? body.data : null;
}
...
function getTicketInfo(festivalId, ticketId, sessionCookie, purchaseSession) {
    ...
    const checkRes = check(response, {
        'status is 200': (r) => r.status === 200,
        'response has purchase ticketInfo': (r) => {
            try {
                const body = JSON.parse(r.body);
...
// 티켓 결제 상태 확인 polling
function getPurchaseStatus(festivalId, ticketId, sessionCookie, email, paymentId) {
    const response = http.get(`${BASE_URL}/festivals/${festivalId}/tickets/${ticketId}/purchase/${paymentId}/status`, {
        headers: {
            'Content-Type': 'application/json',
            'Cookie': sessionCookie
        },
    ...
    const checkRes = check(response, {
        ...
        'purchase successful': (r) => {
            try {
                ...
                console.error('Failed to parse getPurchaseStatus response:', e);
                return false;
            }
        },
    });
    ...
    if (!checkRes) {
        console.error('Get Purchase Status failed:', response.status, response.body, email);
    }
        ...
        if (festivalListData && festivalListData.content && festivalListData.content.length > 0) {
            ...
            if (festivalDetailResult !== null && ticketListResult !== null) {
                ...
                if (sessionInfo != null) {
                    ...
                    sleep(0.5);
                    if (ticketInfo) {
                        let data = purchaseTicket(sessionInfo.festivalId, sessionInfo.ticketId, sessionInfo.sessionCookie, user.email,
                            sessionInfo.purchaseSession);
                        sleep(0.5);
                        if (data) {
                            while (true) {
                                let status = getPurchaseStatus(sessionInfo.festivalId, sessionInfo.ticketId, sessionInfo.sessionCookie,
                                    user.email, data.paymentId);
